...
## [v5.8.2] - 2026-03-22
### 🔧 ปิด Martingale ใน TIER_MASTER

- **[FIX]** `config.py`: `TIER_MASTER.MAX_MARTINGALE_STEPS` 2 → **0** (ปิด MG)
  - _Root cause: backtest 293 trades พบ MG recovery WR=41.7% แต่ breakeven ต้อง 76.8% (2x multiplier ทำให้ lose -3 XRP แต่ win ได้แค่ +0.91 XRP) — 36 sequences รวม -49.41 XRP_
  - _Base stake trades กำไร +10.79 XRP แต่ MG trades ทำลาย -13.41 XRP ทำให้ net กลายเป็น -2.62 XRP_

## [v5.8.1] - 2026-03-22
### 🔧 Fix R_75 CALL RSI Window (55-60 → 55-68)

- **[FIX]** `asset_profiles.json`: R_75 และ R_75_HIGH_VOL `call_max` แก้จาก `60.0` → `68.0`
  - _Root cause: call_max ถูก set ผิดเป็น 60.0 ทำให้ CALL signal ที่ RSI 60-68 ถูก POST-AI BLOCK ทั้งหมด (ตรวจพบจาก log 2026-03-22 ว่า RSI 60.9, 61.3, 63.9, 65.2, 65.9 ถูก reject ทั้งหมด)_
  - _Notes ใน asset_profiles และ CLAUDE.md ระบุ call_max=68 มาตลอด แต่ค่าจริงในไฟล์เป็น 60_

## [v5.8.0] - 2026-03-21
### Added
- **Sniper Mode**: New trading mode focused on high-probability setups.
- **ADX Filter**: TREND_FOLLOWING trades now require ADX(14) > 25.
- **Dynamic Stake Scaling**: 1.5x stake for setups with AI Confidence >= 0.88 AND Local Risk Score >= 0.80.
- **Daily Targets**: Absolute profit target (3.0 XRP) and loss limit (4.0 XRP).
### Changed
- **Tighter RSI Windows**: R_75 asset profile RSI windows tightened to 60-70 (CALL) and 30-40 (PUT).
- **Version Bump**: Major upgrade from v5.7.x to v5.8.0.

## [v5.7.9] - 2026-03-21
### 🛡️ Dynamic Guard Loop Prevention
- **[BUGFIX]** `bot.py`: Implemented a `_dynamic_wait_for_trade` flag to prevent the 60-minute penalty from re-triggering immediately after it expires. The bot now requires at least one new trade result to be recorded before it can re-evaluate and potentially re-pause the global win rate guard.
- **[IMPROVE]** `bot.py`: Added explicit logging when the penalty period expires and when the data-lock is released after a new trade.

---

## [v5.7.8] - 2026-03-21
### 🔥 Dynamic Performance Guard & Market Regime Safety

---

## [v5.7.7] - 2026-03-21
### 🔧 Definitive Wait Timeout Extension + Sync [v5.7.6]
- **[IMPROVE]** `bot.py`: Increased `_max_definitive_retries` from 12 to 24 (60s → 120s wait) to allow for slower broker settlement.
- **[DOCS]** `bot.py`: Updated logs and Telegram alerts to accurately reflect the 120s timeout.
- **[SYNC]** Sync with `v5.7.6` internal features: Pending Result logic, analysis cleaner.

---

## [v5.7.6] - 2026-03-20
### ✨ Pending Result Logic Improvements
- **[NEW]** `bot.py`: Added support for "Pending Result" status messages in Telegram and local logs.
- **[IMPROVE]** `bot.py`: Added code to automatically remove "Pending" notes from trade analysis once a definitive result is received.
...
- **[CONFIG]** `MAX_MARTINGALE_STEPS: 0 → 2` in `TIER_MASTER` profile. Sequence: 1.0 → 2.0 → 4.0 XRP (capped by `MAX_STAKE_AMOUNT: 4.0`).
...
### �Ÿ“� Telegram Bridge Stability & Optimization
...
- _Analysis: การสูญ�€สีย�•�ˆอ�€�™ื�ˆอ�‡�€กิ�”�ˆาก RSI_CALL_MAX �—ี�ˆ 60 ยั�‡กรอ�‡สัญญา�“�—ี�ˆอ�ˆอ�™แอ�„ม�ˆ�„�”�‰ �ƒ�™�‚�“ะ�—ี�ˆ RSI อยู�ˆ�—ี�ˆ 58.7 (Neutral) �‹ึ�ˆ�‡�ƒกล�‰�€�„ีย�‡กั�š�‚ี�”�ˆำกั�” �„วรล�” call_max ล�‡�€หลือ 55 �€�žื�ˆอกรอ�‡สัญญา�“ CALL �—ี�ˆอ�ˆอ�™แอออก�ƒ�™�•ลา�”�—ี�ˆมี High Volatility_
...
- _Analysis: การสูญ�€สีย�•�ˆอ�€�™ื�ˆอ�‡�€กิ�”�ˆาก RSI PUT bounds �—ี�ˆหลวม�€กิ�™�„�› �—ำ�ƒห�‰ส�ˆ�‡สัญญา�“ PUT �ƒ�™�Š�ˆว�‡ RSI �—ี�ˆ�„ม�ˆ�€หมาะสม �„วร�€�žิ�ˆม put_min �ˆาก 42.0 �€�›�‡�™ 45.0 �€�žื�ˆอ�ƒห�‰มั�ˆ�™�ƒ�ˆว�ˆา RSI อยู�ˆ�ƒ�™�Š�ˆว�‡�—ี�ˆ�€หมาะสมกว�ˆาสำหรั�šการ�€�—ร�” PUT_
...
- _Analysis: การสูญ�€สีย�•ิ�”�•�ˆอกั�™ 3 �„รั�‰�‡�ƒ�™ R_75 TREND_FOLLOWING strategy �€กิ�”�ˆาก RSI_CALL_MAX �—ี�ˆ 68 ยั�‡สู�‡�€กิ�™�„�› �—ำ�ƒห�‰ยิ�‡สัญญา�“ CALL �ƒ�™�Š�ˆว�‡�—ี�ˆ RSI อยู�ˆ�—ี�ˆ 63.7 �‹ึ�ˆ�‡�ƒกล�‰�€�„ีย�‡กั�š overbought zone และ�•ลา�”มี high volatility (ATR: 0.1538%) �„วรล�” call_max ล�‡�€หลือ 60 �€�žื�ˆอ�ƒห�‰สัญญา�“มี�„วามแม�ˆ�™ยำมาก�‚ึ�‰�™_
...
- _Analysis: Bot มี consecutive loss 2 �„รั�‰�‡�”�‰วย PUT signal �š�™ R_75 �ƒ�™สภาวะ RSI 39.4 (Neutral) แ�•�ˆยั�‡อยู�ˆ�ƒ�™�Š�ˆว�‡ put_min: 39.0 �—ำ�ƒห�‰ signal �œ�ˆา�™�„�”�‰ แม�‰ว�ˆา�ˆะ�„ม�ˆ oversold �žอ �„วร�›รั�š put_min �‚ึ�‰�™�€�›�‡�™ 42.0 �€�žื�ˆอ�ƒห�‰ PUT signal แ�‚�‡�‡แกร�ˆ�‡กว�ˆา�€�”ิม_
...
### �Ÿ•�️ Asset Selector Optimization
...
### �Ÿ�›️ AI Council Reliability & Context
...
### �ŸŽ� Standardized Confidence Thresholds
...
### �Ÿ�›️ Autonomous AI Council (Full Loop)
...
### �Ÿ�—️ Unified AI Engine & Super Prompt
...
### �š™️ Optimization & Network Resilience
...
### �Ÿ�—️ Multi-Profile Routing & Atomic Security (Reliability Upgrade)
...
- **[RELIABILITY] Hot Reloading**: Successfully updated profiles now trigger a `config` module reload, refreshing `ASSET_STRATEGY_MAP` in memory tanpa�•�‰อ�‡ Restart Bot.
...
### �Ÿš€ Adaptive Volatility & Reliability (Major Upgrade)
...
### �Ÿ�� Clean Architecture: POST-AI Blocks
...
### �š™️ Dynamic Asset Configuration
...
### �Ÿ›�️ Trade Execution Resiliency & Logical Fixes
...
### �Ÿ“� Telegram Logs & Stream Integrity
...
### �Ÿ�› Watchdog False Heartbeat Fix
...
### �Ÿ›�️ RL Protection & Definitive Trade Resolution
...
### �š� Execution Latency Reduction & Signal Quality
...
- **[FEATURE] ai_engine.py**: Added **Confluence Guard** as a PRE-AI filter. Checks MACD line alignment with SMA-detected trend �€” if `UPTREND` but `MACD < 0`, or `DOWNTREND` but `MACD > 0`, the signal is skipped as contradictory before wasting an AI API call. Logged as `�Ÿ›‘ PRE-AI SKIP (Confluence Guard)`.



## [v4.1.1] - 2026-02-27

### �Ÿ›�️ Trade Resolution & Reporting Fixes

- **[BUG FIX] telegram_bridge.py**: Fixed a boolean logic flaw where any trade status other than "WIN" or "DRAW" (such as "OPEN" or "UNKNOWN") was incorrectly reported to Telegram as an "�Œ LOSS". The bot now correctly reports "⏳ OPEN" or "⏳ UNKNOWN" when trade settlement is delayed.
...
### �Ÿ›�️ Profile-Aware Asset Scanner & Martingale Override
...
- **[FEATURE] ai_engine.py**: Implemented **Martingale Override** in `analyze_and_decide`. When `mg_step > 0` (recovery state), the Smart Trader / RL strategy blocking mechanism is bypassed. This prevents the critical conflict where `All strategies blocked` cancels Martingale recovery trades. Logged as `�š�️ Martingale Override`.
...
### �Ÿ›�️ ASSET_SCANNER Activation Fix
...
### �Ÿ›�️ Streaming Integrity & Winrate Restoration
...
### �Ÿ›�️ Execution Safety & Analysis Precision
...
### �Ÿ�• Watchdog & Execution Refinements
...
### �Ÿ›�️ Core Reliability & Memory Patch ("Time Bomb" Fixes)
...
### �Ÿ›�️ Stability & Performance Patch
...
### �Ÿš€ Event-Driven Streaming Architecture (Major Upgrade)
...
- **�Ÿ�›️ AI Council Advisory Mode**: New `COUNCIL_REAL_ADVISORY_ONLY` toggle. When enabled for REAL accounts, the Council acts as an advisor without pausing the bot or requiring code approval.

- **�Ÿ›�️ MACD Momentum Exhaustion Guard**: Pre-execution veto to block CALLs when momentum is shrinking and PUTs when momentum is rising.

- **�š� Tick Velocity Guard**: Real-time protection against market spikes. Blocks execution if 5-tick movement > 50% of current ATR.

- **�Ÿ“Š ATR Snapshot**: Integrated raw ATR into the AI decision snapshot for faster execution checks.
...
### �Ÿ’� Real-time XRP/THB Conversion
...
### �Ÿ›�️ Slippage Guard & Core Logic Restoration
...
### �Ÿ›�️ Final Safety & Stability Patch
...
### �š™️ Logic Hardening & Advanced Analytics
...
### �Ÿ›�️ System Optimization & Logic Hardening
...
### �Ÿš€ AI Selectivity & Token Optimization
...
### �Ÿ�›️ AI Council Auto-Fix (Manual Adjustment)
...
### �Ÿ›�️ Enhanced Bet Gate Intelligence
...
### �Ÿ›�️ RSI Minimum & AI Council Restriction
...
- _Analysis: The bot entered a PUT trade when the RSI was 45.5, which, in hindsight, wasn't sufficiently oversold. Increasing `RSI_PUT_MIN` to 52 will require a stronger oversold condition before entering PUT positions, potentially reducing losses in unclear market conditions. การ�€�žิ�ˆม�„�ˆา RSI_PUT_MIN �ˆะ�Š�ˆวย�ƒห�‰มั�ˆ�™�ƒ�ˆ�„�”�‰ว�ˆา�€รา�ˆะ�€�‚�‰าสู�ˆส�–า�™ะ PUT �€มื�ˆอ�•ลา�”อยู�ˆ�ƒ�™ภาวะ�‚ายมาก�€กิ�™�„�›อย�ˆา�‡�Šั�”�€�ˆ�™�€�—�ˆา�™ั�‰�™ �‹ึ�ˆ�‡�ˆะ�Š�ˆวยล�”�„วาม�€สี�ˆย�‡�ƒ�™การ�€�‚�‰าสู�ˆส�–า�™ะ�€ร�‡ว�€กิ�™�„�›_
...
### �Ÿ›�️ RSI Guard & Bet Gate Refinement
...
### �Ÿ›�️ Bet Gate & Confidence Debugging
...
- **�€�žิ�ˆม RSI threshold �€�›�‡�™ 55 สำหรั�šกลยุ�—�˜�Œ AI_MOMENTUM**

- **[CONFIG_CHANGE] config.py:** �€�žิ�ˆม RSI_OVERBOUGHT �ˆาก 60 �€�›�‡�™ 55 �€�žื�ˆอกรอ�‡ momentum signals �ƒห�‰�€�‚�‰ม�‡ว�”�‚ึ�‰�™

- **[CONFIG_CHANGE] config.py:** อั�ž�€�”�— BOT_VERSION �€�›�‡�™ 3.11.12

- _Analysis: การ�‚า�”�—ุ�™�€กิ�”�ˆาก RSI threshold �›ั�ˆ�ˆุ�šั�™ (60) ยั�‡�„ม�ˆ�€�‚�‰ม�‡ว�”�žอ�ƒ�™การกรอ�‡สัญญา�“ momentum �—ี�ˆแ�‚�‡�‡แกร�ˆ�‡ �—ำ�ƒห�‰�€�‚�‰า trade �ƒ�™�Š�ˆว�‡�—ี�ˆ momentum อ�ˆอ�™แอ การ�€�žิ�ˆม�€�›�‡�™ 55 �ˆะ�Š�ˆวยกรอ�‡�ƒห�‰�€หลือ�€�‰�žาะสัญญา�“�—ี�ˆมี momentum �Šั�”�€�ˆ�™มาก�‚ึ�‰�™_
...
- _Analysis: RSI threshold �›ั�ˆ�ˆุ�šั�™อา�ˆ�ˆะ�•�ˆำ�€กิ�™�„�› �—ำ�ƒห�‰�€กิ�”สัญญา�“�‹ื�‰อ�‚ายมาก�€กิ�™�„�›�ƒ�™สภาวะ overbought การ�€�žิ�ˆม threshold �ˆะ�Š�ˆวยกรอ�‡สัญญา�“�—ี�ˆ�„ม�ˆ�”ีออก�„�›_
...
- **�€�žิ�ˆม�„�ˆา RSI threshold �€�›�‡�™ 75 �€�žื�ˆอล�”สัญญา�“�—ี�ˆอ�ˆอ�™แอ**

- **[CONFIG_CHANGE] config.py:** �€�žิ�ˆม RSI_OVERBOUGHT �ˆาก 65 �€�›�‡�™ 75 �€�žื�ˆอกรอ�‡สัญญา�“ momentum �—ี�ˆแ�‚�‡�‡แกร�ˆ�‡�‚ึ�‰�™

- **[CONFIG_CHANGE] config.py:** อั�ž�€�”�— BOT_VERSION �€�›�‡�™ v3.11.10 สำหรั�šการ�›รั�š RSI threshold

- _Analysis: �ˆากการวิ�€�„ราะห�Œ loss �ƒ�™ AI_MOMENTUM strategy �š�™ 1HZ50V �ž�šว�ˆา RSI threshold �›ั�ˆ�ˆุ�šั�™ (65) ยั�‡หลวม�€กิ�™�„�› �—ำ�ƒห�‰�€�‚�‰า position �ƒ�™�Š�ˆว�‡�—ี�ˆ momentum ยั�‡�„ม�ˆแ�‚�‡�‡แกร�ˆ�‡�žอ การ�€�žิ�ˆม�€�›�‡�™ 75 �ˆะ�Š�ˆวยกรอ�‡สัญญา�“�ƒห�‰แ�‚�‡�‡แกร�ˆ�‡�‚ึ�‰�™_
...
- **�€�žิ�ˆม RSI threshold �€�›�‡�™ 35 �€�žื�ˆอหลีก�€ลี�ˆย�‡สัญญา�“�—ี�ˆอ�ˆอ�™แอ**

- **[CONFIG_CHANGE] config.py:** �€�žิ�ˆม RSI_OVERSOLD �ˆาก 25 �€�›�‡�™ 35 �€�žื�ˆอหลีก�€ลี�ˆย�‡การ�€�‚�‰า�€�—ร�”�ƒ�™สัญญา�“�—ี�ˆอ�ˆอ�™แอ

- **[CONFIG_CHANGE] config.py:** อั�›�€�”�• BOT_VERSION �€�›�‡�™ v3.11.9

- _Analysis: �ˆากการวิ�€�„ราะห�Œการสูญ�€สีย�ƒ�™ R_75 �ž�šว�ˆา RSI �—ี�ˆ 29.5 และ 44.8 ยั�‡�„�‡อ�™ุญา�•�ƒห�‰�€�‚�‰า�€�—ร�”�ƒ�™ส�–า�™การ�“�Œ�—ี�ˆสัญญา�“�„ม�ˆแ�‚�‡�‡แกร�ˆ�‡�žอ การ�€�žิ�ˆม RSI threshold �ˆะ�Š�ˆวยกรอ�‡สัญญา�“�—ี�ˆอ�ˆอ�™แอออก�„�›_
...
- _Analysis: �šอ�—�€สีย�€�‡ิ�™ 3 �€�—ร�”�•ิ�”�•�ˆอกั�™�”�‰วยกลยุ�—�˜�Œ AI_MOMENTUM �š�™ R_75 แส�”�‡ว�ˆา AI confidence threshold สู�‡�€กิ�™�„�›และ�•�‰อ�‡�€�›ิ�” safety guards �€�žิ�ˆม�€�•ิม_
...
### �Ÿ�›️ AI Council Robustness

- **Enhanced Fuzzy Matching:** Improved `_locate_snippet` to ignore comments and normalize quotes/spacing. This resolves the recurring "�Œ Validation Failed: Could not find snippet" errors, particularly in `config.py`.
...
### �Ÿš€ Performance & Stability (Log-Audit Fixes)
...
### �ŸŽ� AI Council: Direct Provider Targeting
...
### �Ÿ’� AI Council: Consultation Mode
...
### �Ÿ“� Telegram: Account Type Visibility

- **Command /status Enhanced**: Now displays whether the bot is running on a **�Ÿ�� DEMO** or **�Ÿ’� REAL** account.
...
### �Ÿ�›️ AI Council: User-Directed Account Control
...
### �Ÿ�›️ AI Council: Sandbox Strategy (TIER_MASTER)
...
### �Ÿ�›️ Telegram: AI Council Remote Control
...
### �Ÿ�›️ Telegram: Improved /sumlog Context
...
### �Ÿ�›️ AI Council: Integrated Project Map
...
### �Ÿ�� AI Council Knowledge & Structure Awareness
...
### �Ÿ�›️ AI Council Telegram Notifications
...
### �Ÿ“� Telegram Trade Notifications
...
### �Ÿ›�️ Safety & Optimization (Data-Driven)
...
### �Ÿ�› Bugfix: MTF (Multi-TimeFrame) Always Failing
...
### �Ÿ�� AI Analyst: Reduce Excessive HOLD Signals
...
### �Ÿ”� Config: Restore AI_CONFIDENCE_THRESHOLD
...
### �š� Performance: Remove OLLAMA from Heavy Routes
...
### �Ÿ“ˆ Signal Quality Improvements
...
### �Ÿ”� Log Analysis Fixes & Win Rate Improvements
...
### �Ÿ�›️ No-Trade Timeout �†’ AI Council
...
- **Status Bar Re-design:** Merged the Asset info and Idle Countdown into a single, non-flickering status line: `�Ÿ“� [1HZ100V] TIER_2 | ⏳ Idle: Scan in 4m 20s... �—�`.
...
### �Ÿ“ˆ Strategy Optimization (RSI Update)
...
### �Ÿš€ Major Features (Ollama Integration)
