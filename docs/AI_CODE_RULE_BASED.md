...
- **Council Sandbox Rule**: ทุกการตัดสินใจปรับ parameter ใน `config.py` ต้องกระทำที่ `TIER_MASTER` profile เท่านั้น

- **Profile Switching**: AI Council สามารถเปลี่ยน `ACTIVE_PROFILE = "TIER_MASTER"` ได้หากต้องการให้การตั้งค่ามีผลทันที

- **ห้ามแก้ Profile อื่น**: ห้ามแก้ไขค่าใน `TIER_MICRO`, `TIER_MINI`, ฯลฯ (ถือเป็น Golden Profiles)

- ห้ามเพิ่ม `AMOUNT` เกิน 5.0 ใน `TIER_MASTER`
...
- ✅ เปลี่ยน parameter สำคัญอื่นๆ นอกเหนือจาก TIER_MASTER (หากมีการระบุเจาะจงโดยผู้ใช้)
