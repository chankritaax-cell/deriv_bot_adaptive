...
> **v5.5.1 (Adaptive)** | A high-performance, asynchronous trading bot designed specifically for **Deriv.com** (Volatility Indices) using **Python (asyncio)**, **WebSocket Streaming**, and **Adaptive Volatility Profiles**.

## 🌟 Key Features

*   **⚡ Event-Driven Core:** Transitioned to a real-time WebSocket streaming model for millisecond data synchronization.
*   🧠 **Hybrid AI Engine (v5.5.1)**: Combines **Google Gemini 2.0** (Analysis), **OpenAI ChatGPT** (Bet Gate), **Claude** (Risk Manager/Council), and **Adaptive Volatility Regimes** for maximum intelligence.
...
*   **🎯 Sniper Recovery System (v5.5.1)**: Dynamic AI confidence thresholds that scale with Martingale steps (Base: 0.75, MG1: 0.80, MG2+: 0.80).
